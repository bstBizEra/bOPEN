...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "evals/results/static-eval-report.json")
    args = parser.parse_args()

    checks: list[dict[str, object]] = []
    checks.append(run([sys.executable, "scripts/validate_package.py"]))

    with tempfile.TemporaryDirectory(prefix="bopen-skill-eval-") as tmp:
        tmp_path = Path(tmp)
        for artifact_type in ("research-report", "architecture-design", "adr", "conformance-review", "implementation-control"):
            output = tmp_path / f"{artifact_type}.md"
            result = run([
                sys.executable,
            ...
            if result["pass"] and (not output.exists() or "TEST-001" not in output.read_text(encoding="utf-8")):
                result["pass"] = False
