...
case "$(cd "$(dirname "$WORKSPACE_ROOT")" && pwd)/$(basename "$WORKSPACE_ROOT")" in
...
for key in ('source_id','repository_url','commit','default_branch','reference_release','license_observed','license_sha256','lockfile','lock_sha256'):
    print(d[key])
PY
)
...
LOCK_HASH="$LOCK_HASH" OPERATOR_ID="$OPERATOR_ID" TARGET="$TARGET" python3 - "$EVIDENCE/clone-metadata.json" <<'PY'
...
d={
  ...
  'actual_commit':os.environ['PIN'],'reference_release':os.environ['RELEASE'],'archived_observed':False,
