...
| `CRED-TOKEN-CONTEXT-SIGNING` | Signing key for the context token carrying `sub`, `tid`, `mid`, `roles`, `scopes` | key ID | external secret manager | `BOPEN_CONTEXT_TOKEN_KEY` | Security Authority | `not_provisioned` — required by `WP-P35-03` |

`CRED-TOKEN-CONTEXT-SIGNING` is the highest-value secret in the eventual system. It attests
which tenant a request belongs to; forging it defeats every isolation control below it,
including the RLS policies. It must be provisioned in an external manager and never in
`.env.local` beyond local development.
