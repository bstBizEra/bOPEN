"""Regression: GOV-P0-02 document-manifest --check must be date-invariant (reproducible
across UTC days) yet still content-sensitive. Before the fix, `generated` was stamped from
wall-clock UTC, so a byte-frozen candidate went stale at every UTC-midnight rollover with
zero content change, breaking exact-SHA reproducibility.
"""
...
class DocumentManifestReproducibilityTests(unittest.TestCase):
    def test_check_is_date_invariant(self):
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp) / "manifest.json"  # absolute -> excluded from its own scan
