...
class UnsignedProposalIsNotSignableTests(unittest.TestCase):
    ...
    def test_shipped_proposal_rejects_in_closure_mode(self):
        ...
        # The invariant this test protects is that the shipped proposal is REJECTED in closure mode.
        # The reason moved earlier in the chain once PG-P0-CLOSURE-001 was recorded consumed
        # (operator ruling, 2026-07-29): PG-P0-CONSUMED-DECISIONS.json changed c176b854... ->
        # d416ea4b..., so the proposal's bound consumed_state_digest is stale and
        # CONSUMED_STATE_MISMATCH now fires before successor-blob resolution is reached.
        # SUCCESSOR_BLOBS_UNRESOLVED is still true of the proposal (successor_tree is "UNRESOLVED");
        # it is simply no longer the FIRST failure. The proposal is unsignable for strictly more
        # reasons than before, not fewer.
        self.assertEqual(cm.exception.reason, v.CONSUMED_STATE_MISMATCH)
